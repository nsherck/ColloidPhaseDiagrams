...
def PMFColCol(FPPMFName = "PMF_Neg3.txt", chi=3.0, outFile = 'PMFColCol3.table'):
	...
	******** Editions and Revisions **********
	...
	Revised 2019.06.09 by Daniel Chu  - Added potential at distance of 0 for LAMMPS file
	Revised 2019.06.10 by Daniel Chu  - Changed to extrapolation to distance of 0
	"""
    ...
    zrange = [(i-16) * (FlagStatSeg * NumStatSeg ** 0.5 + (1 - FlagStatSeg)) for i in zrange]
    ...
    znew3 = [zmin - i*dx for i in range(int(zmin/dx),0,-1)] + znew2
    if znew2[0] == 1:
        ColColCombPot = [ColColCombPot[1] + ColColCombForce[1] * (zmin - znew3[i]) for i in range(0, len(znew3))
                         if znew3[i] <= zmin] + ColColCombPot[1:]
        ColColCombForce = [ColColCombForce[1] for i in znew3 if i <= zmin] + ColColCombForce[1:]
    else:
        ColColCombPot = [ColColCombPot[0] + ColColCombForce[0]*(zmin-znew3[i]) for i in range(0,len(znew3))
                     if znew3[i] < zmin] + ColColCombPot
        ColColCombForce = [ColColCombForce[0] for i in znew3 if i < zmin] + ColColCombForce
    ...
    ColColComb = np.column_stack((index, znew3, ColColCombPot, ColColCombForce))  # combine all for input into lammps
    ...
    f = open(outFile, "w")
    f.write('\n')
    f.write('Col \n')  # necessary for identification in LAMMPS
    cnt = 0
    for i in ColColComb:
        if i[2] <= PMFMaxVal:
            if cnt == 0:
                zmin = i[1]
                f.write('N ' + str(int(ColColComb[-1][0])) + ' R ' + str(ColColComb[0][1]) + ' ' + str(zmax) + '\n')
                f.write('\n')
            cnt = cnt + 1
            f.write('%6i %.10e %.10e %.10e \n' % (cnt, i[1], i[2]/chi, i[3]/chi))
    f.close()

    plt.figure()
    plt.plot(znew3, [i[2] for i in ColColComb], 'r-', lw=2, label="Combined")
    ...
    for i in range(0,len(index)):
        ColColComb[i][3] = ColColComb[i][3]
        ColColComb[i][2] = ColColComb[i][2]
    B2HS = 2 * math.pi / 3
...
if __name__ == "__main__":  # Code that runs the program if called from the command line
    ...
    parser.add_argument('-i', '--inFile', default="PMF_Neg3.txt", type=str, help='Text file for field theory simulation results.')
    parser.add_argument('-c', '--chi', default=3, type=float, help='Chi value of simulation.')
    parser.add_argument('-o', '--outFile', default='PMF_Neg3.table', type=str, help='Output table file.')

    # Parse the command-line arguments
    args = parser.parse_args()
    PMFColCol(args.inFile,args.chi,args.outFile)
